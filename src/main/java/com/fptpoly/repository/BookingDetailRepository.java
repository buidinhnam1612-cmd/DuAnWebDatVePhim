...
public class BookingDetailRepository {
    ...
    public List<String> findSeatBookedByShowtime(String maSuatChieu) {
        ...
        String sql = """
                ...
                INNER JOIN DAT_VE dv ON ct.MaDatVe = dv.MaDatVe
                WHERE ct.MaSuatChieu = ?
                  AND (
                       dv.TrangThai = N'Đã thanh toán'
                       OR (dv.TrangThai = N'Chờ thanh toán' AND ct.ThoiGianGiuGhe > GETDATE())
                  )
                """;
