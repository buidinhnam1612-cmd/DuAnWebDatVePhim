        ...
        <div class="col-md-9 ms-sm-auto col-lg-10 px-md-4 py-4">
            ...
            <!-- ==================== FORM THÊM NHÂN VIÊN ==================== -->
                        <div class="card content-card mb-4">
                            <div class="card-header bg-white py-3">
                                <h5 class="mb-0 fw-bold"><i class="bi bi-person-plus text-success me-2"></i>Thêm Tài Khoản Nhân Viên</h5>
                            </div>
                            <div class="card-body">
                                <form action="${pageContext.request.contextPath}/admin/employee" method="post">
                                    ...
                                    <div class="row g-3">
                                        <div class="col-md-3">
                                            ...
                                            <!-- Đã sửa: Khóa nhập liệu, hiển thị nhãn tự động sinh mã -->
                                            <input type="text" class="form-control bg-light text-muted" name="maNhanVien" value="Mã tự động sinh..." readonly>
                                        </div>
                                        <div class="col-md-3">
                                        ...
                                        <div class="col-md-3">
                                            ...
                                            <!-- Đã sửa: Bỏ lựa chọn tạo Admin, chỉ cho phép tạo 2 nhóm nhân sự tại quầy -->
                                            <select class="form-select" name="maVaiTro">
                                                <option value="VT02">Nhân viên bán vé</option>
                                                <option value="VT04">Nhân viên quầy đồ ăn</option>
                                            </select>
                                        </div>
            ...
            if(employeeList!=null && !employeeList.isEmpty()){
                for(Employee e : employeeList){
                    boolean isAdmin = "VT01".equals(e.getMaVaiTro());
            %>
                                            <tr>
                                                ...
                                                <td class="text-center">
                                                    <span class="badge <%= isAdmin ? "bg-danger" : "bg-info text-dark" %>"><%=e.getTenVaiTro()%></span>
                                                </td>
                                                <td class="text-center">
                                                    <span class="badge <%= "Hoạt động".equals(e.getTrangThai()) || "Đang làm việc".equals(e.getTrangThai()) ? "bg-success" : "bg-danger" %>"><%=e.getTrangThai()%></span>
                                                </td>
                                                <td class="text-center">
                                                    <div class="d-flex align-items-center justify-content-center gap-2">

                                                        <!-- Form chỉnh sửa vai trò chức vụ nhanh -->
                                                        <form action="${pageContext.request.contextPath}/admin/employee" method="post" class="d-flex align-items-center gap-1 mb-0">
                                                            ...
                                                            <select name="maVaiTro" class="form-select form-select-sm" style="width: 150px;" <%= isAdmin ? "disabled" : "" %>>
                                                                <% if(isAdmin) { %>
                                                                    <option value="VT01" selected>Admin</option>
                                                                <% } else { %>
                                                                    <option value="VT02" <%= "VT02".equals(e.getMaVaiTro()) ? "selected" : "" %>>Nhân viên bán vé</option>
                                                                    <option value="VT04" <%= "VT04".equals(e.getMaVaiTro()) ? "selected" : "" %>>Nhân viên đồ ăn</option>
                                                                <% } %>
                                                            </select>
                                                            <button type="submit" name="action" value="updateRole" class="btn btn-sm btn-outline-primary btn-action" title="Đổi quyền" <%= isAdmin ? "disabled" : "" %>><i class="bi bi-person-gear"></i></button>
                                                        </form>

                                                        <!-- Form cập nhật trạng thái hoạt động nhanh -->
                                                        <form action="${pageContext.request.contextPath}/admin/employee" method="post" class="d-flex align-items-center gap-1 mb-0">
                                                            <input type="hidden" name="maNhanVien" value="<%=e.getMaNhanVien()%>">
                                                            <select name="trangThai" class="form-select form-select-sm" style="width: 125px;" <%= isAdmin ? "disabled" : "" %>>
                                                                <option value="Đang làm việc" <%= "Đang làm việc".equals(e.getTrangThai()) || "Hoạt động".equals(e.getTrangThai()) ? "selected" : "" %>>Đang làm việc</option>
                                                                <option value="Khóa" <%= "Khóa".equals(e.getTrangThai()) ? "selected" : "" %>>Khóa</option>
                                                            </select>
                                                            <button type="submit" name="action" value="updateStatus" class="btn btn-sm btn-outline-success btn-action" title="Cập nhật TT" <%= isAdmin ? "disabled" : "" %>><i class="bi bi-check2-circle"></i></button>
                                                        </form>

                                                        <!-- Nút mở khối phân quyền chi tiết dưới chân bảng -->
                                                        <!--
                                                        <% if (!isAdmin) { %>
                                                        <a href="${pageContext.request.contextPath}/admin/employee?action=editPermissions&maNhanVien=<%=e.getMaNhanVien()%>"
